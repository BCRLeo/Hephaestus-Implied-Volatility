...
The file is saved in a folder called "outputs" with the file name format:
    "<ModelName>-<Ticker>-<Years>years-<ForecastDays>days-<timestamp>.png"
...
def run_aparch(returns, forecast_horizon=1):
    ...
    forecast = res.forecast(horizon=forecast_horizon)
    # Extract forecasted variance using .iloc[-1, 0] (as in your provided code)
...
def plot_volatility(model_name, hist_vol, forecast_series, ticker, years_span, forecast_horizon):
    ...
    The title is augmented with ticker, time span, forecast horizon, and timestamp.
...
def main():
    ...
    initial_window = 365  # Adjusted from 30 to 365
    
    # Prompt for forecast horizon (used in the title; rolling forecast always computes one-step forecasts)
    try:
    ...
    # Recursive loop for display options
    while True:
        ...
        if display_choice == "4":
            ...
            # New: Evaluate forecast for each model in option 2
            for model_name, forecast_series in forecast_dict.items():
            ...
            plot_multiple_forecasts(forecast_dict, hist_vol, ticker, years_input, forecast_horizon, same_fig=same_fig)
            # New: Evaluate forecast for each model in option 3
            for model_name, forecast_series in forecast_dict.items():
                print(f"Evaluation for model {model_name}:")
                evaluate_forecast(forecast_series, hist_vol)
